              ...
              (`(application-outer-k ,rand ,env ,k*)
                (let* ((k*^ (alloc state))
                  ...
                  (add-next-state wl state seen-states state-adjacency
                    ...
                    (lambda (wl state-adjacency) (loop ks wl state-adjacency)))))
...
(define apply-proc
  (lambda (ps as k* state wl seen-states state-adjacency k)
    (let loop ((ps ps) (wl wl) (state-adjacency state-adjacency))
      (if (set-empty? ps)
          (k wl state-adjacency)
          (let ((p (set-first ps)))
            (match p
              (`(closure ,x ,body ,env)
    ...
    (if (set-empty? wl)
        ...
        (let* ((state (set-first wl))
          ...
          (match (cdr (state-exp state))
            ((? void? v) (apply-k* state (set v) wl seen-states state-adjacency)) ;; needed to allow apply-k through here.
            ((? number? n) (apply-k* state (set n) wl seen-states state-adjacency))
            ((? boolean? b) (apply-k* state (set b) wl seen-states state-adjacency))
            (`(quote ,d) (apply-k* state (set d) wl seen-states state-adjacency))
            ((? symbol? x)
              (apply-k* state
...
(define list-aux
  (lambda (e* state wl seen-states state-adjacency k)
    (if (null? e*)
