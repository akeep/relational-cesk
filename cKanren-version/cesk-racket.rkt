  ...
  (lambda (k v/s v/s^)
    (conde
      ...
      [(fresh (k^ p s^ cont v-out-ignore)
         ...
         (apply-proco p cont s^ k^ v/s^ v-out-ignore))]
      [(fresh (v-e env^ cc s cont v-out-ignore)
         (== k `(throw-k ,v-e ,env^))
         (make-continuationo cc cont)
         (== v/s `(,cont . ,s))
         (eval-exp-auxo v-e env^ s cc v/s^ v-out-ignore))]
      [(fresh (c a env k^ v s v-out)
         (== k `(if-k ,c ,a ,env ,k^ ,v-out))
         (== v/s `(,v . ,s))
         (conde
           [(=/= v #f) (eval-exp-auxo c env s k^ v/s^ v-out)]
           [(== v #f) (eval-exp-auxo a env s k^ v/s^ v-out)]))]
      [(fresh (k^ v s ans v-out)
         (== k `(zero?-k ,k^ ,v-out))
         (== v/s `(,v . ,s))
      ...
      [(fresh (k^ v s ans v^ v-out)
         ...
         (== v^ v-out)
         (apply-ko k^ ans v/s^))]
      [(fresh (v1 k^ v2 s ans v^ v-out)
         (== k `(subtraction-inner-k ,v1 ,k^ ,v-out))
         (== v/s `(,v2 . ,s))
         (infd v1 (range least-fixnum greatest-fixnum))
         (infd v2 (range least-fixnum greatest-fixnum))
         (infd v^ (range least-fixnum greatest-fixnum))
         (plusfd v^ v2 v1)  ; y = v1 - v2 => y + v2 = v1
         (answero v^ s ans)
         (== v^ v-out)
         (apply-ko k^ ans v/s^))]
      [(fresh (e2 env k^ v s k^^ v-out v-out-ignore)
      ...
      [(fresh (v1 k^ v2 s^^ ans v^ v-out)
         ...
         (== v^ v-out)
         (apply-ko k^ ans v/s^))]
      [(fresh (e2 env k^ v1 s^ k^^ v-out v-out-ignore)
         (== k `(addition-outer-k ,e2 ,env ,k^ ,v-out))
         (== v/s `(,v1 . ,s^))
         (addition-inner-ko v1 k^ v-out k^^)
         (eval-exp-auxo e2 env s^ k^^ v/s^ v-out-ignore))]
      [(fresh (v1 k^ v2 s^^ ans v^ v-out)
      ...
      [(fresh (e2 env k^ v1 s^ k^^ v-out v-out-ignore)
         ...
         (== v/s `(,v1 . ,s^))
         (multiplication-inner-ko v1 k^ v-out k^^)
         (eval-exp-auxo e2 env s^ k^^ v/s^ v-out-ignore))]
      [(fresh (x env k^ v s^ loc ans s^^ v-out)
      ...
      [(fresh (k^ v2 s^^ ans)
         ...
         (answero v2 s^^ ans)
         (apply-ko k^ ans v/s^))]
      [(fresh (rand2 env k^ v1 s^ k^^ v-out)
      ...
      [(fresh (p k^ a s^^ v-out)
         ...
         (== v/s `(,a . ,s^^))
         (apply-proco p a s^^ k^ v/s^ v-out))]
      [(fresh (rand env k^ p s^ k^^ v-out v-out-ignore)
         (== k `(application-outer-k ,rand ,env ,k^ ,v-out))
         (== v/s `(,p . ,s^))
         (application-inner-ko p k^ v-out k^^)
         (eval-exp-auxo rand env s^ k^^ v/s^ v-out-ignore))]
      [(fresh (v k^ v* s^^ ans)
         (== k `(list-aux-inner-k ,v ,k^))
         (== v/s `(,v* . ,s^^))
         (answero (cons v v*) s^^ ans)
         (apply-ko k^ ans v/s^))]
      [(fresh (e* env k^ v s^ a d k^^ v-out)
  ...
  (lambda (exp env s k v/s^ v-out)
    (fresh ()
      ...
      (conde
        ...
        [(fresh (rator rand k^ v-out-ignore)
           (== exp `(,rator ,rand))
...
(define evalo
  (lambda (e o)
    (eval-expo e empty-env empty-store empty-k o)))
